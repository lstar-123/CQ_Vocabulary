  ...
  if (allWordsGrouped.length === 0) {
    container.innerHTML = `
      <div style="text-align:center;padding:64px 24px;color:var(--text-muted);">
        <div style="display:flex;justify-content:center;margin-bottom:16px;color:var(--sage);">${icon('book', 48)}</div>
      ...
      <div class="study-word-list">
        ${g.words.map((w, i) => `
          <div class="study-word-item">
            ...
            <button class="btn-speak-sm" data-word="${escapeHtml(w.english).replace(/"/g, '&quot;')}" title="朗读">${icon('volume-1', 15)}</button>
